...
describe("processBuildingSlots (unit — sans Emulator)", () => {
  ...
  it("1. Calcul correct des cycles complétés", () => {
    ...
    const result = processBuildingSlots(building, inventory, now);
  ...
  it("2. lastProcessedAt = now exactement (déterminisme vérifiable)", () => {
    // Le `now` injecté doit être la référence exacte posée sur le slot —
    // pas une copie reconstituée depuis toMillis(). Vérifie que l'identité objet est préservée.
    const nowMs = Date.now();
    const now = admin.firestore.Timestamp.fromMillis(nowMs);
    ...
    expect(result.updatedSlots[0].lastProcessedAt).toBe(now);
  });
  ...
  it("5. Pas de mutation des arguments d'entrée (inventory inchangé après l'appel)", () => {
    ...
    processBuildingSlots(building, inventory, now);
  ...
  it("7. Recipe inconnue → throw Error (pas HttpsError, le handler convertit)", () => {
    ...
    expect(() => processBuildingSlots(building, inventory, now))
