...
| D-ETAPE17.A-2 | Parsing canonique des Timestamp Firebase côté client Unity | `ParseTimestampMs(object raw) → long` : SOURCE DE VÉRITÉ unique pour toutes les CFs futures (contrats, marché, upgrades — ne jamais créer un second parser). Clés wire : `"_seconds"` / `"_nanoseconds"` **avec underscore** (`@google-cloud/firestore` Timestamp sérialise ses champs internes via `Object.entries`, pas `toJSON()`). `null → 0L`. Formule : `_seconds * 1000L + _nanoseconds / 1_000_000L`. `level` et `slotIndex` arrivent en `long` boxé → toujours `(int)GetLong(...)`, jamais cast direct du dictionnaire. `BuildingSnapshot.Id` = identifiant cible pour toute action client (startProductionSlot, collectProduction, upgrades futurs) — jamais `BuildingType`. Validé en Play : DebugScreen affiche `sawmill_0 (sawmill lv1)`, slot[0/1/2] idle. | Audit 17.A-2 : sonde `Object.keys(Timestamp)` → `['_seconds','_nanoseconds']` + lecture `encode()` dans `firebase-functions/lib/common/providers/https.js` (itère `Object.entries` sans transformation). Sans underscore = null silencieux → timer cassé. Un seul helper = une seule surface de maintenance si Firebase change la sérialisation. | Si Firebase change le format wire (ex. adoption de `toJSON()`) → mettre à jour `ParseTimestampMs` uniquement. |
| D-ETAPE17.A-3 | ProductionScreen — premier écran gameplay branché via ScreenManager | (1) `ProductionScreen : BaseScreen` placé en scène (inactif au départ), activé par `ScreenManager.SetRoot(_productionScreen)` au step 7 d'AppBootstrap, en remplacement de l'activation ad-hoc de DebugScreen (qui reste en scène, inactif, ré-activable pour debug). Premier usage réel du ScreenManager. (2) `startProductionSlot` renvoie `{ building: BuildingDocument; inventory: InventoryState }` — PAS `void`. Le client rend directement l'état serveur renvoyé, aucun re-`resolveLoginState` après le start. (3) `IProductionService`/`FirebaseProductionService` créés sur le patron `FirebasePlayerService` (D-ETAPE17.0). `recipeId` figé à `"logs"`. (4) `ParseTimestampMs` passé `private → internal` dans `PlayerStateSnapshot.cs` pour réutilisation par `ProductionResult.Parse` — source unique préservée (D-ETAPE17.A-2), aucun second parser. (5) buildingId absent de la réponse → injecté par le service (D-ETAPE16-004), jamais reconstruit par convention. | Tâche 0 a confirmé la réponse = 2 écritures (building.slots + player.inventory), aucune autre collection → rendu direct suffisant. Fork tranché en faveur de ScreenManager (propre, conforme C3/C9, déjà câblé) : 17.A-3 = premier écran gameplay = moment d'établir le pattern de navigation. Validé en Play : Slot 0 → « Logs en cours », un seul resolveLoginState au boot, 0 erreur, état persistant au cycle pause/resume (server-side confirmé). | Timer cosmétique réel → quand les durées de recette seront exposées client (Remote Config ou réponse CF) ; repoussé 17.A-4/17.B. Extraction de `ParseTimestampMs` vers un helper dédié au 3e consommateur. `_screenContainer` à câbler si ScreenManager doit un jour instancier des écrans. |
| D-ETAPE17.B-0 | collectProduction renvoie l'état mis à jour | Enrichissement de `{ collected, discarded }` → `{ collected, discarded, building, inventory }`, alignement sur startProductionSlot (décision structurante 17.A-3). Building SANS id Firestore — service client injecte (D-ETAPE16-004). Coût marginal : la transaction lisait déjà `buildingData` et `updatedInventory` avant ce changement, aucun read/compute supplémentaire. Logique temporelle `lastProcessedAt` INTOUCHÉE (TD-004 traité séparément). 204/204 tests verts, grep `^[+-].*lastProcessedAt = 0`. | Diagnostic 17.B a établi que le retour initial `{ collected, discarded }` seul interdisait au client d'afficher l'état post-collecte sans re-`resolveLoginState`, ce qui violerait la doctrine posée en 17.A-3 (INTERDIT dans ProductionScreen.cs L23). Alignement backend préalable au câblage client = sous-étape 17.B-0. | N/A — contrat définitif. Le client cable `CollectProductionAsync` à l'étape 17.B-1 en réutilisant le même patron que `StartProductionSlotAsync`. |
| D-ETAPE-TD004 | Résolution TD-004 — factory injection + flag write-detection | `createTimestamp: (ms: number) => firestore.Timestamp` injectée en 4e param de `processBuildingSlots`. `hasProcessedCycles: boolean` remplace le détecteur par identité d'objet `=== now`. `lastProcessedAt = createTimestamp(reference.toMillis() + completedCycles * durationMs)` (résiduel préservé). | Helper pur (pas d'`import * as admin`) : la factory Timestamp est fournie par le call site. Identité d'objet brisée par `fromMillis(...)` → flag explicite obligatoire. Option B confirmée : `lastProcessedAt` avance de N cycles basés sur le temps, pas sur le yield effectif. | Si Phase 2 introduit des Timestamp précis inter-services, vérifier que `createTimestamp` reste injectée (pas d'import admin hardcodé dans le helper). |
| D-ETAPE17.B | Câblage client de collectProduction (service + bouton « Collecter ») | Service : `IProductionService.CollectProductionAsync(buildingId, ct)` → `CollectProductionResult { Building, Inventory, Collected, Discarded }`, miroir de StartProductionSlotAsync. Helpers de parsing extraits dans `ProductionParsingHelpers` (couche partagée client) ; ProductionResult + CollectProductionResult délèguent — AUCUN second parser, `ParseTimestampMs` reste single-source (internal, PlayerStateSnapshot). UI : UN bouton « Collecter » global au bâtiment (collectProduction opère sur tous les slots), visible si ≥1 slot actif, élément UXML persistant ré-activé via `SetEnabled(true)` dans RenderSlots (non recréé comme les boutons de slot → reset explicite après SetAllButtonsEnabled(false)). Cible `_currentBuilding.Id`, jamais BuildingType. | Handlers renvoient l'état mis à jour, aucun re-resolveLoginState (cohérent 17.A-3 / 17.B-0). Extraction déclenchée par la 2e occurrence du parsing (YAGNI). | TD-015 ouverte (copies privées résiduelles dans PlayerStateSnapshot). |

---

*Dernière mise à jour : 2026-06-18 — ÉTAPE 17.B câblage client collectProduction*
