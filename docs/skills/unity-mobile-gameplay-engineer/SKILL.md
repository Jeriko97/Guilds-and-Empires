...
description: >
   Client-side Unity gameplay engineer for Guilds & Empires (GAE), a mobile-first MMORPG economy game. Use this skill for ANY client Unity implementation decision: C# client architecture, Service Locator, service interfaces, MainThreadDispatcher, threading, async/await, CancellationToken, Firebase Unity SDK consumption, calling Cloud Functions from the client, client-generated idempotency keys, HttpsError handling, UI Toolkit (UXML/USS), runtime UI, screen flow, navigation, state management, UI reconciliation, optimistic UI, loading/error/empty states, offline persistence, app lifecycle (pause/resume), listener disposal, mobile performance in practice, GC pressure, list virtualization. ALWAYS trigger when the user asks how to implement a client screen, how to call a Cloud Function from Unity, how to wire UI to backend, or how to structure the Unity C# client. Also trigger on mentions of UI Toolkit, UXML, USS, uGUI, MonoBehaviour, ServiceLocator, MainThreadDispatcher, GetHttpsCallable, CallAsync, FirebaseFunctions, client Unity, écran, interface, affichage, navigation, état client, bouton, liste, ticker, in the context of Guilds & Empires.
---
